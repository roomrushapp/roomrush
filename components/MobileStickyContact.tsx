...
type Props = {
  ...
  phone: string | null;
  rent: number;
  /** Selector for the full contact card element — bar hides when this is in view */
...
export default function MobileStickyContact({ listing_id, contact_email, phone, rent, contactCardId }: Props) {
  ...
  return (
    <div
      className={`md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-zinc-200 px-4 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-2px_12px_rgba(0,0,0,0.08)] transition-transform duration-200 ${hidden ? "translate-y-full" : "translate-y-0"}`}
    >
      <div className="flex items-center gap-3">
        {/* Rent label */}
        <div className="shrink-0">
          <p className="text-xs text-zinc-400 leading-none mb-0.5">per month</p>
          <p className="font-display font-bold text-xl text-rose-600 leading-none">€{rent.toLocaleString()}</p>
        </div>
          ...
          <>
            ...
            {hasEmail && (
              <button
                ...
                className="shrink-0 flex items-center justify-center border border-zinc-300 hover:border-zinc-500 text-zinc-600 w-11 h-11 transition-colors"
              >
                <Mail size={18} />
              </button>
            )}
