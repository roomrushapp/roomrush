...
function buildShareText(profile: RoomSeekerProfile, url: string): string {
  const lines: string[] = [];
  ...
  lines.push("");
  lines.push(`View profile on RoomRush:`);
  lines.push(url);

  return lines.join("\n");
}

export default function SeekerShareButton({ profile, profileId }: Props) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);

  const profileUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/room-seekers/${profileId}`
      : `https://getroomrush.de/room-seekers/${profileId}`;

  const shareText = buildShareText(profile, profileUrl);
  ...
  async function handleShare() {
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        ...
        // User cancelled (AbortError) → no error shown
        if (err instanceof Error && err.name === "AbortError") return;
        // Anything else → fall through to modal
      }
    }
    setOpen(true);
  }
  ...
  const tgUrl = `https://t.me/share/url?url=${encodeURIComponent(profileUrl)}&text=${encodeURIComponent(shareText)}`;
          ...
          >
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-zinc-500 text-sm mt-1 leading-snug">
                  Send to listers or WG groups so they can see what {profile.name} is looking for.
                </p>
              </div>
              <button
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                style={{ background: copied ? "rgba(16,185,129,0.15)" : "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", color: copied ? "rgb(52,211,153)" : "white" }}
              >
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-sm font-medium text-white transition-colors"
